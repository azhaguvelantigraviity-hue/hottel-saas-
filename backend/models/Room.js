const mongoose = require('mongoose');

const RoomSchema = new mongoose.Schema({
  hotel:      { type: mongoose.Schema.Types.ObjectId, ref: 'Hotel', required: true },
  roomNumber: { type: String, required: true },
  floor:      { type: Number, default: 1 },
  type:       { type: String, enum: ['Standard Twin','Deluxe King','Deluxe Queen','Suite','Premium Suite','Presidential Suite','Executive'], required: true },
  status:     { type: String, enum: ['available','occupied','reserved','maintenance','cleaning'], default: 'available' },
  baseRate:   { type: Number, required: true },
  extraCharges:{ type: Number, default: 0 },
  bedType:    { type: String },
  maxGuests:  { type: Number, default: 2 },
  maxAdults:  { type: Number, default: 2 },
  maxChildren:{ type: Number, default: 2 },
  ac:         { type: Boolean, default: true },
  smoking:    { type: Boolean, default: false },
  nearLift:   { type: Boolean, default: false },
  view:       { type: String, enum: ['Sea', 'City', 'Garden', 'None'], default: 'None' },
  housekeepingStatus: { type: String, enum: ['clean','dirty','inspect','in-progress'], default: 'clean' },
  maintenanceIssue: { type: String },
  expectedCompletion: { type: Date },
  amenities:  [String],
  description:String,
  images:     [String],
  isActive:   { type: Boolean, default: true },
}, { timestamps: true });

RoomSchema.index({ hotel: 1, roomNumber: 1 }, { unique: true });
RoomSchema.index({ hotel: 1, status: 1 });

module.exports = mongoose.model('Room', RoomSchema);
